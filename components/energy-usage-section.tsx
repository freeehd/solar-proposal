        ...
        <div className="container mx-auto px-4">
          ...
          >
            <Card className="bg-card/50 backdrop-blur border-primary/10">
              <CardHeader>
                <CardTitle className="text-2xl font-bold text-center">Energy Cost Overview</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-2xl font-bold text-primary">${Number(proposalData.monthlyBill || 0).toFixed(2)}</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <TrendingUp className="w-8 h-8 mb-2 text-primary" />
                    <p className="text-sm text-muted-foreground">Estimated in 30 years</p>
                    <p className="text-2xl font-bold text-primary">${finalMonthlyPayment.toFixed(2)}</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <Lightbulb className="w-8 h-8 mb-2 text-primary" />
                    <p className="text-sm text-muted-foreground">Total payments after 30 years</p>
                    <p className="text-2xl font-bold text-primary">${totalPayments.toFixed(2)}</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <Zap className="w-8 h-8 mb-2 text-primary" />
                    <p className="text-sm text-muted-foreground">Average Rate/kWh</p>
                    <p className="text-2xl font-bold text-primary">${proposalData.averageRateKWh || "0"}/kWh</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/50 backdrop-blur border-primary/10">
              ...
              <CardContent>
                <div className="h-[400px] w-full">
                  {data.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{ top: 20, right: 20, bottom: 40, left: 40 }} barGap={0} barSize={20}>
